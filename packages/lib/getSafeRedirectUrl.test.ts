...
vi.mock("./constants", () => ({
  WEBAPP_URL: "https://app.rondevu.org",
...
describe("isSafeUrlToLoadResourceFrom", () => {
  ...
  it("should return true for URLs with same TLD+1 as WEBAPP_URL", () => {
    expect(isSafeUrlToLoadResourceFrom("https://acme.rondevu.org/path")).toBe(true);
    expect(isSafeUrlToLoadResourceFrom("http://dunder.rondevu.org?query=1")).toBe(true);
    expect(isSafeUrlToLoadResourceFrom("https://rondevu.org/path")).toBe(true);
  });
